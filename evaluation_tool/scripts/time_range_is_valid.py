...
def time_range_is_valid(start_evaluation_immediately, evaluation_start, evaluation_end):
    ...
    evaluation_start = datetime.datetime.strptime(evaluation_start, "%Y-%m-%dT%H:%M").replace(second=0, microsecond=0)
    evaluation_end = datetime.datetime.strptime(evaluation_end, "%Y-%m-%dT%H:%M").replace(second=0, microsecond=0)

    if start_evaluation_immediately:
        evaluation_start = datetime_now
        evaluation_end = datetime.datetime.now().replace(second=0, microsecond=0) + datetime.timedelta(days=1)
